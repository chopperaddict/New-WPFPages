...
namespace WPFPages . ViewModels
{
	...
	public static class DapperGeneric<T, U, V>
	{
			...
			using ( IDbConnection db = new SqlConnection ( ConString ) )
			{
				try
				{
					...
					try
					{
						...
						{
							Console . WriteLine ( $"Running Select db.Query" );
							//***************************************************************************************************************//
						...
						else
						{
							...
							Console . WriteLine ( $"Running SP db.Query" );
					...
					try
					{
						...
						else if ( SqlCommand . ToUpper ( ) . Contains ( "SELECT " ) )
						{
							...
							if ( UseFull == true )
							{
								...
								//	Console . WriteLine ( $"Generic query returned {objC . Count}" );
								...
								//	Console . WriteLine ( $"Generic query returned {objD . Count}" );
		...
		public static bool CallGeneric<T> ( T arg )
		{
			int ArgTpe = 0;
		...
		/// <returns></returns>
		public static ObservableCollection<GenericClass> CreateFromDictionary<T>(
			T dict, 
			GenericClass gclass,
			string SqlCommand,
			ref string errormsg
		...
		{
			ObservableCollection<GenericClass>objGclass = new();
			string ConString = ( string ) Properties . Settings . Default [ "BankSysConnectionString" ];
			using ( IDbConnection db = new SqlConnection ( ConString ) )
			{
				...
				{
					Console . WriteLine ($"SQL Db.Query error : [{ex.Message}]");
					errormsg = ex . Message;
				}
			}
			return objGclass;
