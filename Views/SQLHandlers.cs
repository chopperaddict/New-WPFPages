...
namespace WPFPages . Views
{
	public class SQLHandlers
	{
		...
		//	object dummy = null;
		//}
		public async Task<bool> UpdateDbRowAsync ( string CurrentDb, object RowData, int Row = 0)
		{
				...
				try
				{
					...
					else if ( CurrentDb == "CUSTOMER" )
					{
						try
						{
							...
							//They should be as Grid vlaidate entries itself !!
							int z;
							z = Convert . ToInt32 ( cs . Id );
							//					string sndr = sender.ToString();
							z = Convert . ToInt32 ( cs . AcType );
							//Check for invalid A/C Type
							if ( z < 1 || z > 4 )
							{
			...
			// These call functions in our Db classes themseves
			if ( CurrentDb == "BANKACCOUNT" )
			{
...
//						SqlCommand cmd = new SqlCommand ( "UPDATE BankAccount SET BANKNO=@bankno, CUSTNO=@custno, ACTYPE=@actype, BALANCE=@balance, INTRATE=@intrate, ODATE=@odate, CDATE=@cdate where CUSTNO = @custno", con );
...
//						cmd = new SqlCommand ( "UPDATE SecAccounts SET BANKNO=@bankno, CUSTNO=@custno, ACTYPE=@actype, BALANCE=@balance, INTRATE=@intrate, ODATE=@odate, CDATE=@cdate where CUSTNO = @custno", con );
//						cmd . Parameters . AddWithValue ( "@id", Convert . ToInt32 ( ss . Id ) );
//						cmd . Parameters . AddWithValue ( "@bankno", ss . BankNo . ToString ( ) );
//						cmd . Parameters . AddWithValue ( "@custno", ss . CustNo . ToString ( ) );
//						cmd . Parameters . AddWithValue ( "@actype", Convert . ToInt32 ( ss . AcType ) );
...
//						cmd = new SqlCommand ( "UPDATE Customer SET BANKNO=@bankno, CUSTNO=@custno, ACTYPE=@actype, ODATE=@odate, CDATE=@cdate where CUSTNO = @custno", con );
//						cmd . Parameters . AddWithValue ( "@id", Convert . ToInt32 ( ss . Id ) );
//						cmd . Parameters . AddWithValue ( "@bankno", ss . BankNo . ToString ( ) );
//						cmd . Parameters . AddWithValue ( "@custno", ss . CustNo . ToString ( ) );
//						cmd . Parameters . AddWithValue ( "@actype", Convert . ToInt32 ( ss . AcType ) );
...
//							"PHONE=@phone, MOBILE=@mobile, DOB=@dob,ODATE=@odate, CDATE=@cdate where CUSTNO = @custno", con);
...
//						cmd = new SqlCommand ( "UPDATE SecAccounts SET BANKNO=@bankno, CUSTNO=@custno, ACTYPE=@actype, " + "ODATE=@odate, CDATE=@cdate where CUSTNO = @custno", con );
//						cmd . Parameters . AddWithValue ( "@id", Convert . ToInt32 ( cs . Id ) );
//						cmd . Parameters . AddWithValue ( "@bankno", cs . BankNo . ToString ( ) );
//						cmd . Parameters . AddWithValue ( "@custno", cs . CustNo . ToString ( ) );
//						cmd . Parameters . AddWithValue ( "@actype", Convert . ToInt32 ( cs . AcType ) );
...
//			return true;
		...
		public async Task<bool> UpdateDbRow ( string CurrentDb, object RowData )
		{
			...
			await UpdateDbRowAsync ( CurrentDb, RowData );
			return true;
...
//						cmd = new SqlCommand ( "UPDATE BankAccount SET BANKNO=@bankno, CUSTNO=@custno, ACTYPE=@actype, BALANCE=@balance, INTRATE=@intrate, ODATE=@odate, CDATE=@cdate where CUSTNO = @custno", con );
//						cmd . Parameters . AddWithValue ( "@id", Convert . ToInt32 ( cs . Id ) );
//						cmd . Parameters . AddWithValue ( "@bankno", cs . BankNo . ToString ( ) );
//						cmd . Parameters . AddWithValue ( "@custno", cs . CustNo . ToString ( ) );
//						cmd . Parameters . AddWithValue ( "@actype", Convert . ToInt32 ( cs . AcType ) );
